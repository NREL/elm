...
async def extract_ordinance_text_with_llm(doc, text_splitter, extractor):
    ...
    ordinance_text = await extractor.check_for_definition(text_chunks)
    doc.attrs["definition_ordinance_text"] = ordinance_text
...
async def extract_ordinance_values(doc=None, **kwargs):
    ...
    **kwargs
    ...
    """
    # if not attrs.get("cleaned_ordinance_text"):
    # if not doc.attrs.get("definition_ordinance_text"):
    #     msg = (
    ...
    # text = doc.attrs["definition_ordinance_text"]
    text = 'test'
    doc.attrs["ordinance_values"] = await parser.parse(text)
    return doc

async def rag_extact_values():
    pass